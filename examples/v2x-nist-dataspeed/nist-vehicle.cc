...
void
CheckMobility(NodeContainer nodes, Time periodicity)
{
  ...
  Simulator::Schedule (periodicity, &CheckMobility, nodes, periodicity);
}
...
void sendToGateway(NistVehicleGateway *gateway, string receiverId, Ptr<const Packet> pkt, const Address &addr)
{
  std::cout<<"Packet for " << receiverId << " received at "<<Simulator::Now()<<"\n";
  gateway->handleReceive(std::stoi(receiverId));
}
                 ...
                 const Address &dstAddrs, const SeqTsSizeHeader &seqTsSizeHeader)
{
  ...
  std::cout << RngSeedManager::GetRun () << " " << Simulator::Now ().GetSeconds () << " " << node->GetId () << " " << txRx << " " << seq << std::endl;
  ...
  if (txRx == "tx")
  ...
  else
    {
      ...
      if (s_rxs.find (node->GetId ()) == s_rxs.end ())
        {
          //mobility->SetVelocityAndAcceleration (Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
...
main (int argc, char *argv[])
{
   ...
   */
  uint16_t numVehiclesPerLane = 1;
  uint16_t numLanes = 1;
  uint16_t interVehicleDist = 20; //meters
  uint16_t interLaneDist = 4; //meters
  s_speed = 33.33; //meter per second, default 120 km/h
  s_deceleration = 33.33; // The rate at which to declerate in meters per second
  ...
  double centralFrequencyBandSl = 5.98e9; // band n47  TDD //Here band is analogous to channel
  ...
  */
  ...
  cmd.AddValue ("interLaneDist",
                ...
                interLaneDist);
  cmd.AddValue ("processingDelay",
                "The processing delay before BSM message is sent in seconds",
                processingDelay);
  cmd.AddValue ("speed",
                "speed of the vehicles in m/sec",
                s_speed);
  cmd.AddValue ("deceleration",
                "deceleration rate of the vehicles in m/sec",
                s_deceleration);
  ...
  cmd.AddValue ("gatewayAddress",
                "IPv4 address for the ros-ns3-gateway server",
                gatewayAddress);
  cmd.AddValue("transmitOnBrake",
                "set if the vehicle transmits a BSM when braking",
                transmitOnBrake);
   ...
   */
  allSlUesContainer = InstallHighwayMobility (numLanes, numVehiclesPerLane, interVehicleDist, interLaneDist, 0);
  //allSlUesContainer = InstallHighwayMobility (numLanes, numVehiclesPerLane, interVehicleDist, interLaneDist, s_speed);

  NodeContainer digitalInfrastructureNodes;
  digitalInfrastructureNodes.Create(3);

  Ptr<ListPositionAllocator> fixedNodeCoordinates = CreateObject<ListPositionAllocator>();
  fixedNodeCoordinates->Add(Vector(395.5,316.5,0)); // Laptop at 101
  fixedNodeCoordinates->Add(Vector(-295,205,0));    // Cell Tower #1
  fixedNodeCoordinates->Add(Vector(45,-250,0));     // Cell Tower #2

  MobilityHelper mobility;
  mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
  mobility.SetPositionAllocator(fixedNodeCoordinates);
  mobility.Install(digitalInfrastructureNodes);  

  allSlUesContainer.Add(digitalInfrastructureNodes);
  ...
  // all nodes can both transmit and receive
      ...
      if (!txSlUes.Contains (node->GetId ()))
        {
          ...
          txSlUes.Add (node);
          txSlUesNetDevice.Add (node->GetDevice (0));
  ...
  sidelinkClient.SetAttribute ("OnTime", StringValue("ns3::ConstantRandomVariable[Constant=1]"));
  sidelinkClient.SetAttribute ("OffTime", StringValue("ns3::ConstantRandomVariable[Constant=0]"));
  std::string dataRateBeString  = std::to_string (dataRateBe) + "kb/s";
  sidelinkClient.SetAttribute ("DataRate", StringValue(dataRateBeString));
  ...
  for (uint16_t i = 0; i  < txSlUes.GetN (); i++) {
      ...
      Time appStart = slBearersActivationTime + Seconds (jitter) + Seconds (processingDelay);
      
      clientApps.Get (i)->SetStartTime (Seconds(6000.0)); // need to set an unreachable time to prevent early transmits

      // clientApps.Get (i)->toggleDynamicScheduling();

      //onoff application will send the first packet at :
      //slBearersActivationTime + random jitter + ((Pkt size in bits) / (Data rate in bits per sec))
      realAppStart =  slBearersActivationTime.GetSeconds () + jitter + ((double)udpPacketSizeBe * 8.0 / (DataRate (dataRateBeString).GetBitRate ())) + processingDelay;
      realAppStopTime = realAppStart + simTime.GetSeconds ();
      
      // clientApps.Get (i)->SetStopTime (simTime);
      
      txAppDuration = realAppStopTime - realAppStart;
      
      std::cout << RngSeedManager::GetRun () << " " << realAppStart << " " << txNodeId << " " << "ev" << " " << "0" << std::endl;
      //Simulator::Schedule (Seconds (realAppStart - processingDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, allSlUesContainer.Get (txNodeId)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
      ...
      for (uint16_t ac = 0; ac < serverApps.GetN (); ac++)
        {
          ...
          serverApps.Get (ac)->TraceConnect("Rx", std::to_string(ac), MakeBoundCallback (&sendToGateway, &gw));
        }
    }
  else
    {
  ...
  if (generateGifGnuScript)
    ...
    auto orchestrator =
        CreateObject<netsimulyzer::Orchestrator>("nist-vehicle.json");
    // Use helper to define model for visualizing nodes and aggregate to Node object

    NodeContainer::Iterator it = allSlUesContainer.Begin();
    netsimulyzer::NodeConfigurationContainer nodeConfigurations;

    // car (index = 0) configuration
    netsimulyzer::NodeConfigurationHelper carConfigurationHelper{orchestrator};
    carConfigurationHelper.Set("Model", netsimulyzer::models::CAR_VALUE);
    carConfigurationHelper.Set("Offset", Vector3DValue(Vector3D(0, 0, -1.5)));
    carConfigurationHelper.Set("Orientation", Vector3DValue(Vector3D(0, 0, 90)));
    carConfigurationHelper.Set("Scale", DoubleValue(2.0));

    netsimulyzer::NodeConfigurationContainer carNodes = carConfigurationHelper.Install(*it);
    carNodes.Get(0)->SetAttribute("Name", StringValue("AV"));
    nodeConfigurations.Add(carNodes);

    // laptop (index = 1) configuration
    ++it;
    netsimulyzer::NodeConfigurationHelper laptopConfigurationHelper{orchestrator};
    laptopConfigurationHelper.Set("Model", netsimulyzer::models::LAPTOP_VALUE);
    laptopConfigurationHelper.Set("Orientation", Vector3DValue(Vector3D(0, 0, -90)));
    laptopConfigurationHelper.Set("Scale", DoubleValue(2.5));

    netsimulyzer::NodeConfigurationContainer laptopNodes = laptopConfigurationHelper.Install(*it);
    laptopNodes.Get(0)->SetAttribute("Name", StringValue("RSU"));
    nodeConfigurations.Add(laptopNodes);

    // cell tower (index = [2,3]) configuration
    netsimulyzer::NodeConfigurationHelper digitalInfrastructureHelper{orchestrator};
    digitalInfrastructureHelper.Set("Model", netsimulyzer::models::CELL_TOWER_VALUE);

    for (int i = 1; ++it != allSlUesContainer.End(); i++) {
      netsimulyzer::NodeConfigurationContainer towerNodes = digitalInfrastructureHelper.Install(*it);
      towerNodes.Get(0)->SetAttribute("Name", StringValue("Cell Tower " + std::to_string(i)));
      nodeConfigurations.Add(towerNodes);
    }
    ...
    // draw the map plane
    auto map = CreateObject<netsimulyzer::Decoration>(orchestrator);
    map->SetPosition({63.0, -370.0, 0.0});
    map->SetOrientation({0.0, 0.0, -1.0});
    // This path is relative to the "resources/" directory
    map->SetAttribute("Model", StringValue{"map-plane.obj"});
    map->SetAttribute("Width", netsimulyzer::OptionalValue<double>(800.0));
    ...
    *eventLog << "Hard breaking event to occur at " << (realAppStart - processingDelay) << "s for RV-1 with a processing delay of " << processingDelay << " s\n";

  //Simulator::Schedule (Seconds (1.0), &CheckMobility, allSlUesContainer, Seconds (0.1));

  // Simulator::Stop (simStopTime);
  // Simulator::Stop (Seconds(5));
  gw.initialize(gatewayAddress.c_str(), 1111, clientApps, nodeConfigurations);
  gw.setTransmitOnBrake(transmitOnBrake);
  Simulator::Run ();
