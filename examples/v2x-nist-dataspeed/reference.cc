...
static uint8_t s_useCase = 1; //1: communication-only, 2: perception-only, 3: hybrid (RV2 -> perception/HV -> communication)
static uint16_t numVehiclesPerLane = 3;
static uint16_t numLanes = 1;
static double s_speed;
static double s_deceleration;
static double s_txProcessingDelay = 0.0;
static double s_rxProcessingDelay = 0.0;
static double s_t0PerceptionReactionDelay = 1.0; //1: alert, 1.5: average, 2: tired, 2.5: mediocre
static double s_t1PerceptionReactionDelay = 1.0; //1: alert, 1.5: average, 2: tired, 2.5: mediocre
static double s_t2PerceptionReactionDelay = 1.0; //1: alert, 1.5: average, 2: tired, 2.5: mediocre
static double s_tnPerceptionReactionDelay = 1.0; //1: alert, 1.5: average, 2: tired, 2.5: mediocre
static double s_mobilityGranularity = 100; // in milliseconds
static double s_avgVehicleLength = 5; // average vehicle length in meters

#ifdef HAS_NETSIMULYZER
static std::vector<Ptr<netsimulyzer::XYSeries> > s_pktDelaySeries;
#endif
static std::unordered_map<uint64_t, Time> s_txs;
static std::unordered_map<uint64_t, Time> s_rxs;

void
CheckMobility(NodeContainer nodes, Time periodicity, double avgVehicleLength)
{
  double xInitPos = nodes.Get (nodes.GetN () - 1)->GetObject<ConstantAccelerationMobilityModel> ()->GetPosition ().x;
  double yInitPos = nodes.Get (nodes.GetN () - 1)->GetObject<ConstantAccelerationMobilityModel> ()->GetPosition ().y;

  for (uint32_t idx = 0; idx < nodes.GetN (); idx++)
    {
      Ptr<Node> node = nodes.Get (nodes.GetN () - 1 - idx);
      Ptr<ConstantAccelerationMobilityModel> mobility = node->GetObject<ConstantAccelerationMobilityModel> ();
      if (mobility->GetVelocity ().x <= 0)
        {
          mobility->SetVelocityAndAcceleration (Vector (0.0, 0.0, 0.0), Vector (0.0, 0.0, 0.0));
        }
      //Crash condition
      if (idx != 0 && mobility->GetPosition ().y == yInitPos && mobility->GetPosition ().x >= xInitPos - avgVehicleLength)
        {
          //Position the following car at the crash site
          mobility->SetPosition (Vector (xInitPos - avgVehicleLength, yInitPos, mobility->GetPosition ().z));
          mobility->SetVelocityAndAcceleration (Vector (0.0, 0.0, 0.0), Vector (0.0, 0.0, 0.0));
          //Stop the car upfront from continuing course
          nodes.Get (nodes.GetN () - idx)->GetObject<ConstantAccelerationMobilityModel> ()->SetVelocityAndAcceleration (Vector (0.0, 0.0, 0.0), Vector (0.0, 0.0, 0.0));
        }
      xInitPos = mobility->GetPosition ().x;
      yInitPos = mobility->GetPosition ().y;
    }

   Simulator::Schedule (periodicity, &CheckMobility, nodes, periodicity, avgVehicleLength);
}
                 ...
                 const Address &dstAddrs, const SeqTsSizeHeader &seqTsSizeHeader)
{
  ...
  std::cout << RngSeedManager::GetRun () << " " << RngSeedManager::GetSeed () << " " << Simulator::Now ().GetSeconds () << " " << node->GetId () << " " << txRx << " " << seq  << " " << seqTsSizeHeader.GetTs ().GetSeconds () << std::endl;

  Ptr<ConstantAccelerationMobilityModel> mobility = node->GetObject<ConstantAccelerationMobilityModel> ();

  if (txRx == "tx")
    {
      s_txs[seq] = Simulator::Now ();
    }
  else
    {
      #ifdef HAS_NETSIMULYZER
      if (numVehiclesPerLane == 3 && numLanes == 1)
      {
        s_pktDelaySeries[node->GetId ()]->Append (Simulator::Now ().GetSeconds (), (Simulator::Now () - s_txs[seq]).GetSeconds ());
      }
      #endif
      if (s_rxs.find (node->GetId ()) == s_rxs.end ())
        {
          Simulator::Schedule (Seconds (s_rxProcessingDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, mobility, Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
          s_rxs[node->GetId ()] = Simulator::Now ();
        }
    }
}
...
/**
 * \brief Method to listen the application level traces of type TxWithAddresses
 *        and RxWithAddresses for background traffic.
 * \param stats Pointer to the UeToUePktTxRxOutputStats class,
 *        which is responsible to write the trace source parameters to a database.
 * \param node The pointer to the TX or RX node
 * \param localAddrs The local IPV4 address of the node
 ...
 */
void
BgUePacketTraceDb (UeToUePktTxRxOutputStats *stats, Ptr<Node> node, const Address &localAddrs,
                 std::string txRx, Ptr<const Packet> p, const Address &srcAddrs,
                 const Address &dstAddrs, const SeqTsSizeHeader &seqTsSizeHeader)
{
  uint32_t nodeId = node->GetId ();
 ...
 */
NodeContainer
InstallHighwayMobility (uint16_t totalLanes, uint16_t numVehiclesPerLane, double interVehicleDist, uint16_t interLaneDist, double speed)
{
  ...
  for (int i = 0; i < totalLanes * numVehiclesPerLane; i++)
    {
      ueNodes.Get (i)->GetObject<ConstantAccelerationMobilityModel> ()->SetVelocityAndAcceleration (Vector (speed, 0.0, 0.0), Vector (0.0, 0.0, 0.0));
    }
...
/**
 * \brief Install random mobility
 * \param numUes the number of UEs with background traffic 
 * \param interVehicleDist The distance between the vehicles in a same lane
 * \param interLaneDist The distance between the lanes, i.e., the distance
 *        between the vehicles of two different lanes
 * \param speed The speed of the vehicles
 * \return A node container containing the background vehicle UEs with their mobility model
 *         installed
 */
NodeContainer
InstallBackgroundMobility (uint16_t numUes, double interVehicleDist, uint16_t interLaneDist, double speed)
{
  NodeContainer ueNodes;
  ueNodes.Create (numUes);
  NS_ABORT_MSG_IF ((numUes) % 2 != 0, "The number of UEs should be even!"); //creating pairs
  MobilityHelper mobility;
  mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
                                 ...
                                 "Z", DoubleValue (1.6),
                                 "DeltaX", DoubleValue (interVehicleDist),
                                 "DeltaY", DoubleValue (interLaneDist),
...
/**
 * \brief Record mobility of the vehicle UEs every millisecond
...
void RecordMobility (bool FirstWrite, std::string fileName)
{
  ...
  Simulator::Schedule (MilliSeconds (s_mobilityGranularity), &RecordMobility, FirstWrite, fileName);
}
...
main (int argc, char *argv[])
{
   ...
   */
  uint16_t interVehicleDist = 50; //meters
  uint16_t interLaneDist = 4; //meters
  s_speed = 16; //meter per second
  s_deceleration = 10; // The rate at which to declerate in meters per second^2
  //double interVehicleTime = 1; //3, 2, 1, 0.5 seconds
  bool logging = false;

  // Traffic parameters (that we will use inside this script:)
  bool useIPv6 = false; // default IPV4
  uint32_t udpPacketSizeBe = 389;
  double reTxTimer = 100.0; // The retransmission timer in milliseconds
  uint32_t reTxCount = 10; // The number of of retransmissions to perform 
 
  //Background traffic 
  bool bgTrafficEnabled = false;
  uint16_t bgUes = 2; //number of UEs exchanging background traffic: creating pairs
  uint32_t bgPacketSize = 200; //bytes
  double bgDataRate = 16; //16 kilobits per second
  ...
  double centralFrequencyBandSl = 5.915e9; // band n47  TDD //Here band is analogous to channel
  ...
  double slProbResourceKeep = 0.8;
  uint16_t slMaxTxTransNumPssch = 1;
  ...
  cmd.AddValue ("logging",
                "Enable logging",
                logging);
  cmd.AddValue ("mobilityGranularity",
                "Position and mobility granularity",
                s_mobilityGranularity);
  cmd.AddValue ("avgVehicleLength",
                "The length of vehicles in meters",
                s_avgVehicleLength);
  cmd.AddValue ("numVehiclesPerLane",
                "Number of vehicles per lane", 
                numVehiclesPerLane);
  cmd.AddValue ("numLanes", 
                "Total Number of lanes going from West to East", 
                numLanes);
  cmd.AddValue ("interVehicleDist",
                ...
                interVehicleDist);
  /*cmd.AddValue ("interVehicleTime",
                "The time between two vehicles: used to calculate the inter-vehicle distance based on speed",
                interVehicleTime);*/
  cmd.AddValue ("interLaneDist",
                ...
                interLaneDist);
  cmd.AddValue ("txProcessingDelay",
                "The transmission processing delay before BSM message is sent in seconds",
                s_txProcessingDelay);
  cmd.AddValue ("rxProcessingDelay",
                "The reception processing delay after BSM message is received in seconds",
                s_rxProcessingDelay);
	cmd.AddValue ("t0PerceptionReactionDelay",
                "The perception reaction delay for RV-1 in seconds",
                s_t0PerceptionReactionDelay);
  cmd.AddValue ("t1PerceptionReactionDelay",
                "The perception reaction delay RV-2 in seconds",
                s_t1PerceptionReactionDelay);
  cmd.AddValue ("t2PerceptionReactionDelay",
                "The perception reaction delay HV in seconds",
                s_t2PerceptionReactionDelay);
  cmd.AddValue ("tnPerceptionReactionDelay",
                "The perception reaction delay for all cars in between (>= 3 vehicles) in seconds",
                s_tnPerceptionReactionDelay);
  cmd.AddValue ("useCase",
                "1 if communication-only scenario is considered, 2 if perception-only scenario is considered, and 3 if hybrid scenario is considered",
                s_useCase);
  cmd.AddValue ("speed",
                "speed of the vehicles in m/sec",
                s_speed);
  cmd.AddValue ("deceleration",
                "deceleration rate of the vehicles in m/s^2",
                s_deceleration);
  ...
  cmd.AddValue ("packetSizeBe",
                ...
                udpPacketSizeBe);
  cmd.AddValue ("reTxTimer",
                "The retransmission timer in milliseconds (ms)",
                reTxTimer);
  cmd.AddValue ("reTxCount",
                "The number of times to retransmit the original packet",
                reTxCount);
  ...
  cmd.AddValue ("noise",
                "noise figure",
                noise);
  cmd.AddValue ("bgTrafficEnabled",
                "A flag to enable/disable background traffic",
                bgTrafficEnabled);
  cmd.AddValue ("bgUes",
                "Number of UEs with background traffic",
                bgUes);
  cmd.AddValue ("bgPacketSize",
                "UDP packet size for background traffic",
                bgPacketSize);
  cmd.AddValue ("bgDataRate",
                "Date rate for background traffic",
                bgDataRate);
  ...
  //double interVehicleDist = s_speed * interVehicleTime; //meters
   ...
   * Noise figure
   */
  Config::SetDefault ("ns3::NrUePhy::NoiseFigure", DoubleValue(noise));
   ...
   */
  allSlUesContainer = InstallHighwayMobility (numLanes, numVehiclesPerLane, interVehicleDist, interLaneDist, s_speed);

  /**
   * Create a NodeContainer for background UEs (exchanging background traffic) if BG traffic is enabled
   */
  NodeContainer allSlBgUesContainer;
  if (bgTrafficEnabled)
  {
    allSlBgUesContainer = InstallBackgroundMobility (bgUes, interVehicleDist, interLaneDist, s_speed);
  }
  ...
  if (enableChannelRandomness)
    {
      ...
      Config::SetDefault ("ns3::ThreeGppChannelModel::vScatt", DoubleValue(s_speed));
  ...
  NetDeviceContainer allSlBgUesNetDeviceContainer;
  if (bgTrafficEnabled)
  {
    allSlBgUesNetDeviceContainer = nrHelper->InstallUeDevice (allSlBgUesContainer, allBwps);
    for (auto it = allSlBgUesNetDeviceContainer.Begin (); it != allSlBgUesNetDeviceContainer.End (); ++it)
      {
        DynamicCast<NrUeNetDevice> (*it)->UpdateConfig ();
      }
  }
  ...
  if (bgTrafficEnabled)
  {
    nrSlHelper->PrepareUeForSidelink (allSlBgUesNetDeviceContainer, bwpIdContainer);
  }
  ...
  if (bgTrafficEnabled)
  {
    nrSlHelper->InstallNrSlPreConfiguration (allSlBgUesNetDeviceContainer, slPreConfigNr);
  }
  ...
  uint16_t txNodeId = numLanes * numVehiclesPerLane - 1; 
  ...
  NodeContainer txSlBgUes;
  NodeContainer rxSlBgUes;
  NetDeviceContainer txSlBgUesNetDevice;
  NetDeviceContainer rxSlBgUesNetDevice;
  
  if (bgTrafficEnabled)
  {
    stream += nrHelper->AssignStreams (allSlBgUesNetDeviceContainer, stream);
    stream += nrSlHelper->AssignStreams (allSlBgUesNetDeviceContainer, stream);

    for (uint16_t j = 0; j < bgUes; j++)
      {
        //UEs with even IDs are transmitters
        if (j % 2 == 0)
          {
            txSlBgUes.Add (allSlBgUesContainer.Get (j));
            Ptr<NetDevice> dev = allSlBgUesContainer.Get (j)->GetDevice (0);
            Ptr<NrUeNetDevice> ueDev = DynamicCast <NrUeNetDevice> (dev);
            NS_ABORT_MSG_IF (ueDev == nullptr, "Device 0 is not the NrUeNetDevice");
            txSlBgUesNetDevice.Add (allSlBgUesContainer.Get (j)->GetDevice (0));
          }
        //UEs with odd IDs are receivers
        else
          {
            rxSlBgUes.Add (allSlBgUesContainer.Get (j));
            Ptr<NetDevice> dev = allSlBgUesContainer.Get (j)->GetDevice (0);
            Ptr<NrUeNetDevice> ueDev = DynamicCast <NrUeNetDevice> (dev);
            NS_ABORT_MSG_IF (ueDev == nullptr, "Device 0 is not the NrUeNetDevice");
            rxSlBgUesNetDevice.Add (allSlBgUesContainer.Get (j)->GetDevice (0));
          }
      }
  }
  ...
  if (!useIPv6)
    {
      ...
      Ptr<LteSlTft> tftTx = Create<LteSlTft> (LteSlTft::Direction::TRANSMIT, LteSlTft::CommType::GroupCast, groupAddress4, dstL2Id);
      //Set Sidelink bearers
      nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, txSlUesNetDevice, tftTx);

      Ptr<LteSlTft> tftRx = Create<LteSlTft> (LteSlTft::Direction::RECEIVE, LteSlTft::CommType::GroupCast, groupAddress4, dstL2Id);
      //Set Sidelink bearers
      nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, rxSlUesNetDevice, tftRx);
    }
  else
    {
      Ipv6InterfaceContainer ueIpIface = epcHelper->AssignUeIpv6Address (allSlUesNetDeviceContainer);
      ...
      Ptr<LteSlTft> tftTx = Create<LteSlTft> (LteSlTft::Direction::TRANSMIT, LteSlTft::CommType::GroupCast, groupAddress6, dstL2Id);
      //Set Sidelink bearers for transmitting UEs
      nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, txSlUesNetDevice, tftTx);

      Ptr<LteSlTft> tftRx = Create<LteSlTft> (LteSlTft::Direction::RECEIVE, LteSlTft::CommType::GroupCast, groupAddress6, dstL2Id);
      //Set Sidelink bearers for receiving UEs
      nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, rxSlUesNetDevice, tftRx);
    }

  /*
   * Configure the applications for vehicles:
  ...
  double dataRateBe = (udpPacketSizeBe * 8 * (1e3 / reTxTimer)) / 1e3; // Determine how often packet should be sent
  uint32_t maxBytes = udpPacketSizeBe * (reTxCount + 1); // Determine the total number of bytes to send

  //Set Application in the UEs
  OnOffHelper sidelinkClient ("ns3::UdpSocketFactory", remoteAddress);
  sidelinkClient.SetAttribute ("EnableSeqTsSizeHeader", BooleanValue (true));
  sidelinkClient.SetAttribute ("MaxBytes", UintegerValue (maxBytes));
  ...
  for (uint16_t i = 0; i  < txSlUes.GetN (); i++)
    {
      ...
      Time appStart = slBearersActivationTime + Seconds (jitter) + Seconds (s_txProcessingDelay) + Seconds (s_t0PerceptionReactionDelay);
      //onoff application will send the first packet at :
      //slBearersActivationTime + random jitter + ((Pkt size in bits) / (Data rate in bits per sec))
      realAppStart =  slBearersActivationTime.GetSeconds () + jitter + ((double)udpPacketSizeBe * 8.0 / (DataRate (dataRateBeString).GetBitRate ())) + s_txProcessingDelay + s_t0PerceptionReactionDelay;
      realAppStopTime = realAppStart + simTime.GetSeconds ();
      txAppDuration = realAppStopTime - realAppStart;

      switch (s_useCase) {
        case 1: //Communication-only case
          ...
          std::cout << RngSeedManager::GetRun () <<  " " << RngSeedManager::GetSeed () << " " << realAppStart << " " << txNodeId << " " << "ev" << " " << "0" << std::endl;
          Simulator::Schedule (Seconds (realAppStart - s_txProcessingDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, allSlUesContainer.Get (txNodeId)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
          ...
          for (uint16_t j = 0; j  < rxSlUes.GetN (); j++)
            {
              serverApps.Add (sidelinkSink.Install (rxSlUes.Get (j)));
              serverApps.Start (Seconds (0.0));
            }
          break;
        case 2: //perception-only case       
          //HV
          Simulator::Schedule (Seconds (realAppStart - s_txProcessingDelay + s_t2PerceptionReactionDelay + (rxSlUes.GetN () - 2) * s_tnPerceptionReactionDelay + s_t1PerceptionReactionDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, rxSlUes.Get(0)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
          std::cout << "HV" << " starts decelerating at " << std::fixed << std::setprecision (5) << realAppStart - s_txProcessingDelay + s_t2PerceptionReactionDelay + (rxSlUes.GetN () - 2) * s_tnPerceptionReactionDelay + s_t1PerceptionReactionDelay << " sec" << std::endl;

          //RVn
          for (uint16_t j = 1; j < rxSlUes.GetN () - 1; j++)
            {
              Simulator::Schedule (Seconds (realAppStart - s_txProcessingDelay + (rxSlUes.GetN () - j - 1) * s_tnPerceptionReactionDelay + s_t1PerceptionReactionDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, rxSlUes.Get(j)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
              std::cout << "RV " << rxSlUes.GetN () - j + 1 << " starts decelerating at " << std::fixed << std::setprecision (5) << realAppStart - s_txProcessingDelay + (rxSlUes.GetN () - j - 1) * s_tnPerceptionReactionDelay + s_t1PerceptionReactionDelay << " sec" << std::endl;
            } 
          
          //RV2
          Simulator::Schedule (Seconds (realAppStart - s_txProcessingDelay + s_t1PerceptionReactionDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, rxSlUes.Get(rxSlUes.GetN () - 1)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
          std::cout << "RV 2 " << " starts decelerating at " << std::fixed << std::setprecision (5) << realAppStart - s_txProcessingDelay + s_t1PerceptionReactionDelay << " sec" << std::endl;
                     
          //RV1
          Simulator::Schedule (Seconds (realAppStart - s_txProcessingDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, allSlUesContainer.Get (txNodeId)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
          std::cout << "RV " << i + 1 << " starts decelerating at " << std::fixed << std::setprecision (5) << realAppStart - s_txProcessingDelay << " sec" << std::endl;

          break;
        case 3: //hybrid case: RV2 uses perception and HV uses communication
          clientApps.Add (sidelinkClient.Install (txSlUes.Get (i)));
          clientApps.Get (i)->SetStartTime (appStart);
          clientApps.Get (i)->SetStopTime (Seconds (realAppStopTime));
          ...
          //Only the HV car supports communication
          serverApps.Add (sidelinkSink.Install (rxSlUes.Get (0)));
          serverApps.Start (Seconds (0.0));

          //other vehicles support perception
          for (uint16_t j = 1; j < rxSlUes.GetN () - 1; j++)
            {
              //RVn
              Simulator::Schedule (Seconds (realAppStart - s_txProcessingDelay + (rxSlUes.GetN () - j - 1) * s_tnPerceptionReactionDelay + s_t1PerceptionReactionDelay), &ConstantAccelerationMobilityModel::SetVelocityAndAcceleration, rxSlUes.Get(j)->GetObject<ConstantAccelerationMobilityModel> (), Vector (s_speed, 0.0, 0.0), Vector (s_deceleration * -1, 0.0, 0.0));
          ...
          break;
        default:
          std::cout << "Invalid use case: 1: communication-only, 2: perception-only, 3: hybrid (RV2 -> perception/HV -> communication)" << std::endl;
      }
    }

  /**
   * Configure internet for background traffic
   * Configure the applications for UEs with background traffic:
   * Client app: OnOff application configure to generate CBR traffic
   * Server app: PacketSink application.
   */
  uint32_t bgDstL2Id = 222;
  Ipv4Address bgGroupAddress4 ("238.0.0.0");     //use multicast address as destination
  Ipv6Address bgGroupAddress6 ("ff02::1");     //use multicast address as destination
  Address bgRemoteAddress;
  Address bgLocalAddress;
  uint16_t bgPort = 6000;
  //std::string bgOnTime = "ns3::ConstantRandomVariable[Constant=2.0]";
  //std::string bgOffTime = "ns3::ConstantRandomVariable[Constant=1.0]";
  ApplicationContainer bgClientApps;
  ApplicationContainer bgServerApps;

  if (bgTrafficEnabled)
  {
    //Internet setup
    internet.Install (allSlBgUesContainer);
    stream += internet.AssignStreams (allSlBgUesContainer, stream);

    if (!useIPv6)
      {
          Ipv4InterfaceContainer ueBgIpIface = epcHelper->AssignUeIpv4Address (allSlBgUesNetDeviceContainer);
          Ipv4StaticRoutingHelper bgIpv4RoutingHelper;
          for (uint32_t u = 0; u < allSlBgUesContainer.GetN (); ++u)
            {
              Ptr<Node> ueNode = allSlBgUesContainer.Get (u);
              // Set the default gateway for the UE
              Ptr<Ipv4StaticRouting> ueStaticRouting = bgIpv4RoutingHelper.GetStaticRouting (ueNode->GetObject<Ipv4> ());
              ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress (), 1);
            }
        
          bgRemoteAddress = InetSocketAddress (bgGroupAddress4, bgPort);
          bgLocalAddress = InetSocketAddress (Ipv4Address::GetAny (), bgPort);

          Ptr<LteSlTft> bgTftTx = Create<LteSlTft> (LteSlTft::Direction::TRANSMIT, LteSlTft::CommType::GroupCast, bgGroupAddress4, bgDstL2Id);
          nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, txSlBgUesNetDevice, bgTftTx);
          Ptr<LteSlTft> bgTftRx = Create<LteSlTft> (LteSlTft::Direction::RECEIVE, LteSlTft::CommType::GroupCast, bgGroupAddress4, bgDstL2Id);
          nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, rxSlBgUesNetDevice, bgTftRx);
      }
    else
      {
          Ipv6InterfaceContainer ueBgIpIface = epcHelper->AssignUeIpv6Address (allSlBgUesNetDeviceContainer);
          Ipv6StaticRoutingHelper bgIpv6RoutingHelper;
          for (uint32_t u = 0; u < allSlBgUesContainer.GetN (); ++u)
            {
              Ptr<Node> ueNode = allSlBgUesContainer.Get (u);
              // Set the default gateway for the UE
              Ptr<Ipv6StaticRouting> ueStaticRouting = bgIpv6RoutingHelper.GetStaticRouting (ueNode->GetObject<Ipv6> ());
              ueStaticRouting->SetDefaultRoute (epcHelper->GetUeDefaultGatewayAddress6 (), 1);
            }
                
          bgRemoteAddress = Inet6SocketAddress (bgGroupAddress6, bgPort);
          bgLocalAddress = Inet6SocketAddress (Ipv6Address::GetAny (), bgPort);

          Ptr<LteSlTft> bgTftTx = Create<LteSlTft> (LteSlTft::Direction::TRANSMIT, LteSlTft::CommType::GroupCast, bgGroupAddress6, bgDstL2Id);
          nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, txSlBgUesNetDevice, bgTftTx);
          Ptr<LteSlTft> bgTftRx = Create<LteSlTft> (LteSlTft::Direction::RECEIVE, LteSlTft::CommType::GroupCast, bgGroupAddress6, bgDstL2Id);
          nrSlHelper->ActivateNrSlBearer (slBearersActivationTime, rxSlBgUesNetDevice, bgTftRx);
      }

    //client side
    OnOffHelper bgSidelinkClient ("ns3::UdpSocketFactory", bgRemoteAddress);
    bgSidelinkClient.SetAttribute ("EnableSeqTsSizeHeader", BooleanValue (true));
    std::string bgDataRateString  = std::to_string (bgDataRate) + "kb/s";
    bgSidelinkClient.SetConstantRate (DataRate (bgDataRateString), bgPacketSize);
    //bgSidelinkClient.SetAttribute ("OnTime", StringValue (bgOnTime));
    //bgSidelinkClient.SetAttribute ("OffTime", StringValue (bgOffTime));
    for (uint16_t i = 0; i  < txSlBgUes.GetN (); i++)
      {
        bgClientApps.Add (bgSidelinkClient.Install (txSlBgUes.Get (i)));
        bgClientApps.Start (slBearersActivationTime);
      }

    //server side
    PacketSinkHelper bgSidelinkSink ("ns3::UdpSocketFactory", bgLocalAddress);
    bgSidelinkSink.SetAttribute ("EnableSeqTsSizeHeader", BooleanValue (true));
    for (uint16_t i = 0; i  < rxSlBgUes.GetN (); i++)
      {
        bgServerApps.Add (bgSidelinkSink.Install (rxSlBgUes.Get (i)));
        bgServerApps.Start (slBearersActivationTime);
      }
  }
  ...
  UeToUePktTxRxOutputStats bgPktStats;
  bgPktStats.SetDb (&db, "bgPktTxRx");
  ...
  if (bgTrafficEnabled)
  {
    ...
    {
      // Set Tx traces
      for (uint16_t ac = 0; ac < bgClientApps.GetN (); ac++)
        {
          Ipv4Address lAddress =  bgClientApps.Get (ac)->GetNode ()->GetObject<Ipv4L3Protocol> ()->GetAddress (1,0).GetLocal ();
          bgClientApps.Get (ac)->TraceConnect ("TxWithSeqTsSize", "tx", MakeBoundCallback (&BgUePacketTraceDb, &bgPktStats, bgClientApps.Get (ac)->GetNode (), lAddress));
        }
      // Set Rx traces
      for (uint16_t ac = 0; ac < bgServerApps.GetN (); ac++)
        {
          Ipv4Address lAddress =  bgServerApps.Get (ac)->GetNode ()->GetObject<Ipv4L3Protocol> ()->GetAddress (1,0).GetLocal ();
          bgServerApps.Get (ac)->TraceConnect ("RxWithSeqTsSize", "rx", MakeBoundCallback (&BgUePacketTraceDb, &bgPktStats, bgServerApps.Get (ac)->GetNode (), lAddress));
        }
    }
    else
    {
      ...
      for (uint16_t ac = 0; ac < bgClientApps.GetN (); ac++)
        {
          bgClientApps.Get (ac)->GetNode ()->GetObject<Ipv6L3Protocol> ()->AddMulticastAddress (bgGroupAddress6);
          Ipv6Address lAddress =  bgClientApps.Get (ac)->GetNode ()->GetObject<Ipv6L3Protocol> ()->GetAddress (1,1).GetAddress ();
          bgClientApps.Get (ac)->TraceConnect ("TxWithSeqTsSize", "tx", MakeBoundCallback (&BgUePacketTraceDb, &bgPktStats, bgClientApps.Get (ac)->GetNode (), lAddress));
        }
      // Set Rx traces
      for (uint16_t ac = 0; ac < bgServerApps.GetN (); ac++)
        {
          bgServerApps.Get (ac)->GetNode ()->GetObject<Ipv6L3Protocol> ()->AddMulticastAddress (bgGroupAddress6);
          Ipv6Address lAddress =  bgServerApps.Get (ac)->GetNode ()->GetObject<Ipv6L3Protocol> ()->GetAddress (1,1).GetAddress ();
          bgServerApps.Get (ac)->TraceConnect ("RxWithSeqTsSize", "rx", MakeBoundCallback (&BgUePacketTraceDb, &bgPktStats, bgServerApps.Get (ac)->GetNode (), lAddress));
  ...
  //Extend the simulation stop time because it is too short to allow for a full stop of the cars (if the deceleration rate is small)
  Time simStopTime = simTime;

  if (generateGifGnuScript)
    {
      std::string mobilityFileName = "mobility-" + exampleName + ".txt";
      RecordMobility (true, mobilityFileName);
      WriteGifGnuScript (mobilityFileName, simStopTime, s_speed, rxSlUes.Get (0), rxSlUes.Get (rxSlUes.GetN () - 1));
    }
    ...
    if (numVehiclesPerLane == 3 && numLanes == 1)
    {
      ...
      std::vector<std::string> visNodeNames {"HV", "RV-2", "RV-1"};
      auto orchestrator =
          CreateObject<netsimulyzer::Orchestrator>("automated-vehicles.json");
      ...
      nodeHelper.Set("Offset", Vector3DValue(Vector3D(0, 0, -1.5)));
      nodeHelper.Set("Orientation", Vector3DValue(Vector3D(0, 0, 90)));
      netsimulyzer::NodeConfigurationContainer visNodes = nodeHelper.Install(allSlUesContainer);
      for (uint32_t visNodesIdx = 0; visNodesIdx < visNodes.GetN (); visNodesIdx++)
        {
          visNodes.Get (visNodesIdx)->SetAttribute ("Name", StringValue(visNodeNames[visNodesIdx]));
        }
      ...
      PointerValue xAxis; 
      PointerValue yAxis;
      Ptr<netsimulyzer::SeriesCollection> pktDelayCollection = CreateObject <netsimulyzer::SeriesCollection>(orchestrator);
      pktDelayCollection->SetAttribute ("AutoColor", BooleanValue (true));
      pktDelayCollection->SetAttribute ("Name", StringValue("BSM Message Delay"));
      pktDelayCollection->GetAttribute ("XAxis", xAxis);
      xAxis.Get<netsimulyzer::ValueAxis> ()->SetAttribute ("Name", StringValue ("Time (s)"));
      pktDelayCollection->GetAttribute ("YAxis", yAxis);
      yAxis.Get<netsimulyzer::ValueAxis> ()->SetAttribute ("Name", StringValue ("Delay (s)"));

      for (uint32_t visNodesIdx = 0; visNodesIdx < visNodes.GetN (); visNodesIdx++)
        {
          Ptr<netsimulyzer::XYSeries> series = CreateObject <netsimulyzer::XYSeries>(orchestrator);
          series->SetAttribute ("Name", StringValue (visNodeNames[visNodesIdx]));
          series->SetAttribute ("LabelMode", StringValue ("Hidden"));
          series->SetAttribute ("Connection", StringValue ("None"));
          pktDelayCollection->Add (series);
          s_pktDelaySeries.push_back (series);
        }

      *eventLog << "Hard breaking event to occur at " << (realAppStart - s_txProcessingDelay) << "s for RV-1 with a tx processing delay of " << s_txProcessingDelay << " s\n";
      #endif
    }
  Simulator::Schedule (Seconds (1), &CheckMobility, allSlUesContainer, MilliSeconds (s_mobilityGranularity), s_avgVehicleLength);
