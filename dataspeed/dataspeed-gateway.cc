 ...
 */
class DataspeedGateway : public Gateway
{
    public:
        // initialize a simple gateway where n = vehicles.GetN()
        DataspeedGateway(NodeContainer vehicles);
...
DataspeedGateway::DataspeedGateway(NodeContainer vehicles):
...
void
DataspeedGateway::HandleReceive(std::string id, Ptr<const Packet> packet, const Address &clientAddress)
{
...
main(int argc, char* argv[])
{
    ...
    if (verboseLogs)
    {
        ...
        LogComponentEnable("DataspeedGateway", LOG_LEVEL_ALL);
    }
    else
    {
        LogComponentEnable("Gateway", LOG_LEVEL_INFO);
    ...
    DataspeedGateway gateway(vehicles);
    ...
    for (uint32_t i = 0; i < vehicles.GetN(); i++)
    {
        ...
        serverApps.Get(0)->TraceConnect("Rx", std::to_string(i), MakeCallback(&DataspeedGateway::HandleReceive, &gateway));
